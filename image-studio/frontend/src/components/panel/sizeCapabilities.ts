...
const RUNNINGHUB_IMAGE_TO_IMAGE_ASPECT_PRESETS: AspectPresetOption[] = APIMART_ASPECT_PRESETS.filter((item) => (
  ["auto", "1:1", "16:9", "9:16", "4:3", "3:4", "3:2", "2:3", "21:9", "9:21"] as string[]
).includes(item.value));

const RUNNINGHUB_TEXT_TO_IMAGE_ASPECT_PRESETS: AspectPresetOption[] = APIMART_ASPECT_PRESETS.filter((item) => (
  ["auto", "1:1", "3:2", "2:3", "5:4", "4:5", "16:9", "9:16", "21:9", "3:4", "4:3", "9:21", "2:1", "1:2", "3:1", "1:3"] as string[]
).includes(item.value));
...
function runningHubAspectPresets(mode?: Mode | string): AspectPresetOption[] {
  return runningHubMode(mode) === "edit"
    ? RUNNINGHUB_IMAGE_TO_IMAGE_ASPECT_PRESETS
    : RUNNINGHUB_TEXT_TO_IMAGE_ASPECT_PRESETS;
}

function nearestAspectPreset(aspect: AspectPreset, options: AspectPresetOption[]): Exclude<AspectPreset, "auto"> {
  if (aspect === "auto") return "1:1";
  const ratioMatch = String(aspect).match(/^(\d+):(\d+)$/);
  const ratio = ratioMatch ? Number(ratioMatch[1]) / Number(ratioMatch[2]) : 1;
  let best: Exclude<AspectPreset, "auto"> = "1:1";
  let bestDiff = Number.POSITIVE_INFINITY;
  for (const option of options) {
    if (option.value === "auto") continue;
    const diff = Math.abs(Math.log(ratio) - Math.log(option.w / option.h));
    if (diff < bestDiff) {
      best = option.value;
      bestDiff = diff;
    }
...
export function aspectPresetsForAPIMode(apiMode: APIMode, mode?: Mode | string): AspectPresetOption[] {
  if (apiMode === "runninghub") return runningHubAspectPresets(mode);
...
export function normalizeAspectSelection(
  ...
  if (input.apiMode === "runninghub") {
    ...
    return nearestAspectPreset(aspect, runningHubAspectPresets(input.mode));
  }
...
export function buildSizeSelection(
  ...
  if (input.apiMode === "runninghub") {
    return `${nearestAspectPreset(aspect, runningHubAspectPresets(input.mode))}@${normalizedResolution}`;
  }
...
export function buildResolutionSizeSelection(
  ...
  const defaultAspect = input.apiMode === "apimart"
    ...
    : input.apiMode === "runninghub"
      ? DEFAULT_RUNNINGHUB_ASPECT_FROM_AUTO
...
export function sizeCapabilityHint(input: {
  ...
  if (input.apiMode === "runninghub") {
    return "RunningHub 的 2K/4K 请求已按比例+分辨率发出，但上游低价通道仍可能实际返回 1K 图像。";
  }
  if (input.apiMode === "apimart") {
    return "";
  }
