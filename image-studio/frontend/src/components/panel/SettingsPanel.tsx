...
import { clearAllHistory } from "../../state/historyOperations.ts";
...
export function SettingsPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  const {
    ...
    pushToast,
    apiKey, baseURL, apiMode, fhlTransportMode,
  ...
  async function configureFHLFromSettings() {
    ...
    await ensureFHLResponsesProfile(store, { setActive: !isAndroid });
    useStudioStore.getState().pushToast(
      isAndroid
        ? "FHL Responses 文本配置已准备，仅供提示词工具使用。"
        : "已切到 FHL 推荐配置，请粘贴自己的 API Key。",
      "success",
      4200,
    );
  ...
  async function clearHistory() {
    try {
      await clearAllHistory(
        ...
        (count) => confirm(`确定清除 ${count} 条历史记录吗?(本地数据库也会删除)`),
      );
    } catch (e: any) {
      pushToast(`历史清空失败:${e?.message ?? e}`, "error", 5000);
    }
  }
  ...
  async function pruneHistory(days: number) {
    try {
      ...
      pushToast(`历史清理失败:${e?.message ?? e}`, "error", 5000);
    }
  }
