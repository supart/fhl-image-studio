...
type Options struct {
	...
	// NoPromptRevision keeps the normal strict path: ask the text model to pass
	// the prompt to image_generation verbatim. Images API ignores this field.
	NoPromptRevision bool

	// AllowPromptAdaptation is used only by retry paths when the text model
	// replied with text instead of actually calling image_generation.
	AllowPromptAdaptation bool
	...
	// ImagesNewAPICompat forces Images API requests into non-streaming b64_json mode.
